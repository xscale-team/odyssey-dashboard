...
> **Last updated: 2026-04-27 (Session 43)**
> Goal-driven, not timeline-driven. Ship MVP when pipeline is bulletproof.
>
> **What To Do Next:** Supabase Advisor hardening SQL from migrations 063/064 is applied live and the view-reloptions regression passes. After the backend deploy finishes, rerun Advisor for project `kuugnodksduxuccxkyls`, then resume the owner path smoke: "What's my CPA?" → "Make me a batch of ads" → pick product/angle → confirm product image → verify generated ads appear in chat and survive reload. Remaining product gaps: unify bot/dashboard metrics, clean up/offline-reject hidden generated assets from failed streams, reduce idle attach/reconnect flicker after reload, add OCR-backed creative review, require evidence for factual trust claims before ad generation, and make `create_review_batch`/Open Review CTA reliable for generated ads. Full notes: `docs/qa-runs/2026-04-26-brand-owner-ad-bot-qa.md` and `docs/qa-runs/2026-04-26-deep-brand-owner-production-qa.md`.
...
### P1 — Active
- [x] **Supabase Advisor hardening needs live DB apply** - Session 43 applied migrations 063/064 live to project `kuugnodksduxuccxkyls` after user confirmation: `review_batches`/`review_variants` are security-invoker views, SECURITY DEFINER functions have explicit `search_path`, server-only RPCs are service-role-only, broad public storage listing policies are removed, and card/dashboard RLS policies are explicit. Backend now calls privileged RPCs with service-role helpers. View-reloptions regression passes against live DB.
