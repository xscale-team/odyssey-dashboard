...
>
> **What To Do Next:** Supabase Security Advisor is down to 0 errors and 1 warning after live DB/Auth hardening. The remaining warning is `public.pg_net`; live inspection shows `pg_net.extrelocatable = false` and existing `net.*` dependencies, so do not drop/recreate it casually in production. If we want a perfectly clean Advisor, schedule a maintenance window or ask Supabase support for the recommended migration. After the Session 45 verification-tool deploy finishes, resume the owner path smoke in production from a fresh chat: "What's my CPA?" -> "Make me a batch of ads" -> pick product/angle -> verify the actual product image renders before confirmation -> confirm -> verify generated ads appear in chat and survive reload.
