...
| 39 | 2026-04-26 | **Plain owner-path production QA + hidden ad generation fix.** Tested the real owner phrasing the product must handle: "What's my CPA?" then "Make me a batch of ads" then a simple product/angle/persona reply. Found three owner-facing failures: chat CPA reported ~$72 while the home card showed $35.56, the ad path skipped mandatory product-image confirmation and jumped into a 5-ad batch, and the SSE stream dropped with `network error` while the backend kept generating and charging hidden duplicate ad assets that never appeared in chat. Patched the frontend transient reconnect path to attach to the existing stream instead of POST-retrying the same owner message, patched native engine cancellation so detached image-generation tools are cancelled instead of continuing invisibly, passed `conversation_id` into native tool context, persisted future `generated_assets.conversation_id`, and tightened the orchestrator prompt so broad "make me a batch" defaults to a simple 3-ad starter batch after product-image confirmation. Verification: native router/ad-generation tests, frontend API/chat-store tests, Python compile, frontend production build, and `git diff --check` passed. |
